...
function worked_percent($val1,$val2)
{
	$res = 0;
	if($val1 > 0 && $val2 > 0)
	{
		$res = number_format($val1/$val2*100,2);
	}
	return $res;
}

function simple_formated($inum = 0)
{
	$ipembagi = 1000000000; //Miliar
	$ipembagi2 = 100000000; //Seratus Juta
	$ipembagi3 = 1000000; //Sepuluh Juta
	$result = 0;
	
	if($inum > $ipembagi)
	{
		$h = $inum / $ipembagi;
		$result = number_format($h,2) . "M";
	}
	else
	{
		if($inum > $ipembagi2)
		...
		elseif($inum > $ipembagi3)
		{
			$h = $inum / $ipembagi3;
			$result = number_format($h,1) . "JT";
		}
		else 
		{
			$h = $inum;
			$result = number_format($h,2);				
		}
	}
