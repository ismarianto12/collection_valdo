...
class Hdr_spv_report_ctrl extends Controller {
    ...
    public function get_full_report(){//die();
    	$file_now = "RH".date("Y_m_d_Gis").".txt";
    	//$file_now = "RH2022_02_10.txt";
    	$today = date("Y-m-d");
    	//$today = '2022-02-10';
    	$today_fname = date("Y_m_d");
    	//$today_fname = '2022_02_10';
    	//die($filenow);
	...
	//$DBsby = $this->load->database('sby',TRUE);
    	...
    	$command = "INSERT INTO hdr_calltrack_sby(id_contact_code,id_ptp,id_location_code,id_risk_code,id_handling_debt,id_handling_code,deliquency_code,TYPE,ptp_status,primary_1,cname,debtor_name,spv_name,location_code,contact_code,risk_code,next_action_code,id_call_cat,id_action_call_track,code_call_track,id_user,id_spv,username,surveyor,angsuran_ke,remarks,no_contacted,new_phone_number,new_office_number,new_emergency_phone,new_hp,new_address,new_pos_code,memo,date_in,dpd,call_date,call_month,call_time,createdate,total_call_time,ptp_date,ptp_amount,due_date,due_time,call_attempt,incomming,is_current,in_use,sta,cycling,ptp_fu,fu,broken_date,os_ar_amount,kode_cabang)
									...
									FROM adira_collection_sby.hdr_calltrack WHERE createdate >= '$today 00:00:00' and createdate <= '$today 23:59:59'
    							";
    	...
    	$sql = "
    	SELECT 'BRANCH_ID','BRANCH_NAME','CONTRACT_NO','CUSTOMER_NAME','PROMMISED','ACTION','HANDLING_DEBITUR','RESULT_HANDLING','DELIQUENCY','STATUS_UNIT', 'STATUS_DEBITUR', 'KETERANGAN', 'PIC_HANDLING', 'TANGGAL_RESULT','OVERDUE_NUMBER','INST','TM_HANDLING','NO_TELP_DIHUBUNGI'
    	UNION SELECT a.* FROM (SELECT
              ...
              IF(IFNULL(IFNULL(hdc.ptp_date,hdcsby.ptp_date),'') != '0000-00-00',IFNULL(IFNULL(hdc.ptp_date,hdcsby.ptp_date),''),'') AS 'PROMMISED',
	      ...
	      IFNULL(hdc.no_contacted,'') AS 'NO_TELP_DIHUBUNGI'
				...
				LEFT JOIN hdr_action_call_track hda
					...
					ORDER BY tanggal_result desc) a WHERE a.TANGGAL_RESULT != ''					
    ...
    public function get_dataskip($flag){
    	...
    	$file_now2 = "DSH_Payment".date("Y_m_d_Gis").".txt";
    	$file_now3 = "DSH_ACTIVEON".date("Y_m_d_Gis").".txt";
    	$tgl_now = date('Y-m-d');

		$tahun = date('Y');
		$bulan = date('m');
		$tgl = date('d')+2;
		$tgl_2 =$tahun.'-'.$bulan.'-'.$tgl;
...
//flag skip
    	...
    	*/
      	...
      	if($flag == 6)
			{
    	$sql = "
    		SELECT  a.branch, a.primary_1, a.NAME    		FROM hdr_debtor_main a, hdr_payment s WHERE a.primary_1=s.primary_1 and a.primary_1 <> ''
    		and date(s.create_date)='$tgl_now'
    		ORDER BY a.Name
    		INTO OUTFILE '/tmp/$file_now2' FIELDS TERMINATED BY '|' LINES TERMINATED BY '\\r\\n';
    	";
    	//die($sql);
    	$this->db->query($sql);
    	
    	$this->load->helper('download');
      $file_path = '/tmp/' . $file_now2;
      $files_real = file_get_contents($file_path);
      force_download($file_now2, $files_real);
      
			}

      	if($flag == 7)
			{
    	$sql = "SELECT  primary_1, dpd, score_result, object_group, ptp_date as PROMMISED
    		FROM hdr_calltrack where primary_1 <> ''
    		and ptp_date='$tgl_2'
    		ORDER BY cname
    		INTO OUTFILE '/tmp/$file_now3' FIELDS TERMINATED BY '|' LINES TERMINATED BY '\\r\\n';
    	";
    	//die($sql);
    	$this->db->query($sql);

    	$this->load->helper('download');
      $file_path = '/tmp/' . $file_now3;
      $files_real = file_get_contents($file_path);
      force_download($file_now3, $files_real);
...
*/
	...
	public function report_adira_offdulu(){
		...
		public function report_adira_view(){
			...
			$tl 			= $this->uri->segment(7);
			$cabang 		= $this->uri->segment(8);
			$deskcoll 		= $this->uri->segment(9);
			//die($bulan." - ".$tahun." - ".$area." - ".$tl." - ".$cabang." - ".$deskcoll);
			...
			$leader 		= $tl == "hery" ? $leader = 'All' : $leader = $tl;
			$branch 		= $cabang == "hery" ? $branch = 'All' : $branch = $cabang;
			$dc 			= $deskcoll == "hery" ? $dc = 'All' : $dc = $deskcoll;
			$data['month'] 	= $month;
			$data['year'] 	= $year;
			...
			$data['branch'] 	= $branch;
			$data['dc'] 		= $dc;
			//die($month." - ".$year." - ".$location." - ".$leader." - ".$branch." - ".$dc);
			$data['row_daily1'] = $this->report_model->daily_report1($month,$year,$location); //total account mentah
			$data['row_daily2'] = $this->report_model->daily_report1($month,$year,$location); //total account system
			$data['row_daily3'] = $this->report_model->daily_report3($month,$year,$location); //untouch available
			$data['row_daily4'] = $this->report_model->daily_report4($month,$year,$location); //skip
			$data['row_daily5'] = $this->report_model->daily_report5($month,$year,$location); //active on
			//$data['row_daily5'] = $this->report_model->daily_report5($month,$year,$location);
			
			$data['row_daily6'] = $this->report_model->daily_report6($month,$year,$location,$leader,$branch,$dc); //total account called
			$data['row_daily7'] = $this->report_model->daily_report7($month,$year,$location,$leader,$branch,$dc); //Total CONTACT
			$data['row_daily8'] = $this->report_model->daily_report8($month,$year,$location,$leader,$branch,$dc); //Total PTP
			$data['row_daily9'] = $this->report_model->daily_report9($month,$year,$location,$leader,$branch,$dc); //Total PK (Promise Kept)
			$data['row_daily10'] = $this->report_model->daily_report10($month,$year,$location,$leader,$branch,$dc); //Total Uncontact
			
			$data['row_daily11'] = $this->report_model->daily_report11($month,$year,$location,$leader,$branch,$dc); //Total acct Untouch called
			$data['row_daily12'] = $this->report_model->daily_report12($month,$year,$location,$leader,$branch,$dc); //Total acct Untouch CONTACT
			$data['row_daily13'] = $this->report_model->daily_report13($month,$year,$location,$leader,$branch,$dc); //Total acct Untouch PTP
			$data['row_daily14'] = $this->report_model->daily_report14($month,$year,$location,$leader,$branch,$dc); //Total acct Untouch PK (Promise Kept)
			$data['row_daily15'] = $this->report_model->daily_report15($month,$year,$location,$leader,$branch,$dc); //Total acct Untouch Uncontact
			
			$data['row_daily16'] = $this->report_model->daily_report16($month,$year,$location,$leader,$branch,$dc); //MPP (active User)
 ...
 public function get_handling($begindate,$enddate){
    	$file_now = "HANDLING".date("Y_m_d_Gis").".txt";
        //var_dump($enddate);die();
    	/*$sql = "
    	SELECT CASE WHEN a.id_call_cat = 1 THEN 'CONTACT' WHEN a.id_call_cat = 2 THEN 'UNCONTACT' ELSE 'UNDEFINED' END AS CALL_STATUS,
	a.primary_1 AS 'NO_CONTRACT',a.cname AS 'CUST_NAME',date_format(trim(a.call_date),'%d/%m/%Y')  AS 'CALL_DATE', 
	REPLACE(REPLACE(a.remarks,'\r\n',''),'\n','') AS 'CALL_REMARKS',b.area_code AS 'KODE CABANG',
	b.branch_name AS 'BRANCH', a.object_group AS 'OBJECT_GROUP',a.os_ar_amount AS 'OS/AR',
	IFNULL(c.description,CASE a.id_handling_code WHEN '03' THEN 'SALAH SAMBUNG' WHEN '04' THEN 'BUSY LINE' WHEN '05' 
	THEN 'Invalid Number' WHEN '06' THEN 'No Answer' ELSE '' END) AS 'RESULT HANDLING', a.no_contacted AS 'NOTELP',case 
	when left(a.no_contacted,2)='08' then 'Handphone' else 'Rumah' end as ACTION ,
	CASE WHEN a.id_handling_debt THEN p.HAND_DEBT_DESC 
	ELSE 'DEBITUR' END AS HANDLING_DEBITUR,CASE WHEN a.id_handling_debt IN (06, 07, 08 )THEN 'TIDAK ADA' 
	ELSE 'ADA' END AS STATUS_UNIT, CASE WHEN a.id_handling_debt NOT IN (06, 07, 08 )THEN 'ADA' 
	ELSE '' END AS STATUS_DEBITUR, hu.fullname AS FULLNAME FROM hdr_calltrack a
      	LEFT JOIN hdr_branches b
	ON a.kode_cabang = b.area_code
	LEFT JOIN hdr_call_catagory c
	ON a.id_handling_code = c.code
        LEFT JOIN hdr_contact_code s
        on a.id_contact_code = s.code
        LEFT JOIN hdr_calltrack_sby t
        on a.primary_1 = t.primary_1
        LEFT JOIN hdr_handling_debitur_name p
        on a.id_handling_debt = p.HAND_DEBT_CODE
	inner join hdr_user hu on hu.id_user = a.id_user
	WHERE 
	a.call_date BETWEEN '$begindate' AND '$enddate'
	AND a.createdate = (
	SELECT MAX(c.createdate) FROM hdr_calltrack c WHERE c.primary_1 = a.primary_1 AND
	c.call_date BETWEEN '$begindate' AND '$enddate'

	)
	group by a.primary_1
    	";*/
    	
    	$sql="SELECT CASE WHEN a.id_call_cat = 1 THEN 'CONTACT' WHEN a.id_call_cat = 2 THEN 'UNCONTACT' ELSE 'UNDEFINED' END AS CALL_STATUS, hu.fullname AS PIC,
	a.primary_1 AS 'NO_CONTRACT',a.cname AS 'CUST_NAME',date_format(trim(a.call_date),'%d/%m/%Y')  AS 'CALL_DATE', a.CALL_TIME, a.PTP_DATE,a.ANGSURAN_KE,a.dpd ,
	REPLACE(REPLACE(a.remarks,'\r\n',''),'\n','') AS 'CALL_REMARKS',b.area_code AS 'KODE CABANG',
	b.branch_name AS 'BRANCH', a.object_group AS 'OBJECT_GROUP',a.os_ar_amount AS 'OS/AR',
	IFNULL(c.description,CASE a.id_handling_code WHEN '03' THEN 'SALAH SAMBUNG' WHEN '04' THEN 'BUSY LINE' WHEN '05' 
	THEN 'Invalid Number' WHEN '06' THEN 'No Answer' ELSE '' END) AS 'RESULT HANDLING', a.no_contacted AS 'NO_TUJUAN',case 
	when left(a.no_contacted,2)='08' then 'Handphone' else 'Rumah' end as ACTION ,
	CASE WHEN a.id_handling_debt THEN p.HAND_DEBT_DESC 
	ELSE 'DEBITUR' END AS HANDLING_DEBITUR,CASE WHEN a.id_handling_debt IN (06, 07, 08 )THEN 'TIDAK ADA' 
	ELSE 'ADA' END AS STATUS_UNIT, CASE WHEN a.id_handling_debt NOT IN (06, 07, 08 )THEN 'ADA' 
	ELSE '' END AS STATUS_DEBITUR FROM hdr_calltrack a
      	LEFT JOIN hdr_branches b
	ON a.kode_cabang = b.area_code
	LEFT JOIN hdr_call_catagory c
	ON a.id_handling_code = c.code
    ...
    public function report_dataadira() {
        ...
        //$data['list_user'] = $this->user_model->get_list_user($user_cond);
        $this->load->view('spv/hdr_header_spv', $data_t);
        $this->load->view('spv/hdr_spv_data', $data);
        $this->load->view('spv/hdr_footer', $data);
    ...
    public function sum_data() {
       ...
       // $data['list_user'] = $this->user_model->get_list_user($user_cond);
        //die();
      	//$this->report_model->before_call($user="",$id_user="",$status="",$begindate,$enddate);
        $data['total_report'] = $this->report_model->status_total();
//var_dump($data);die();
        $data['all_data'] = $this->report_model->status_data();
        
        //die();
        $this->load->view('spv/hdr_sum_reportdata', $data);
    }
