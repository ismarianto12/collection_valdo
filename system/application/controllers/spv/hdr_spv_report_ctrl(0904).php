...
class Hdr_spv_report_ctrl extends Controller {
	...
	//$DBsby = $this->load->database('sby',TRUE);
    	...
    	$sql = "
    	SELECT 'BRANCH_ID','BRANCH_NAME','CONTRACT_NO','CUSTOMER_NAME','PROMMISED','ACTION','HANDLING_DEBITUR','RESULT_HANDLING','DELIQUENCY','STATUS_UNIT', 'STATUS_DEBITUR', 'KETERANGAN', 'PIC_HANDLING', 'TANGGAL_RESULT','OVERDUE_NUMBER','INST','TM_HANDLING','KATEGORI'
    	UNION SELECT
	      ...
	      case when IFNULL(IFNULL(hdc.id_handling_code,hdcsby.id_handling_code),'') in ('02','01','07','15','08') then 'PROSPEK'
					when IFNULL(IFNULL(hdc.id_handling_code,hdcsby.id_handling_code),'') in ('04','05') then 'NO PROSPEK'
	      else 'INDIKASI' end as 'KATEGORI'
    ...
    function get_dbtrack(){
    	$file_now = "DDB".date("Y_m_d_Gis").".txt";
    	$sql = "SELECT
     CURDATE() AS TGL_REPORT,
     ( CASE WHEN a.valdo_cc = '01' THEN 'JAKARTA' WHEN valdo_cc = '02' THEN 'SURABAYA' ELSE 'UNKNOWN' END ) AS SEGMENT,
     COUNT(id_debtor) AS TOTAL_UPLOADED,
     COUNT( CASE WHEN skip = 1 OR is_paid = 1 THEN id_debtor END ) AS SKIP_OR_PAID,
     COUNT( CASE WHEN skip <> 1 AND is_paid IS NULL AND is_new = 1 THEN id_debtor END) AS UNTOUCH,
     COUNT( CASE WHEN skip <> 1 AND is_paid IS NULL AND is_new = 2 THEN id_debtor END) AS RETOUCH,
     COUNT( CASE WHEN skip <> 1 AND is_paid IS NULL AND is_new = 0 THEN id_debtor END) AS UNCATEGORIZE,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 1) AND (b.`id_action_call_track` IS NOT NULL) THEN id_debtor END) AS UNTOUCH_CALL,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 2) AND (b.`id_action_call_track` IS NOT NULL) THEN id_debtor END) AS RETOUCH_CALL,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 0) AND (b.`id_action_call_track` IS NOT NULL) THEN id_debtor END) AS UNCATEGORIZE_CALL,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 1) AND (b.`id_action_call_track` IS NULL) THEN id_debtor END) AS UNTOUCH_NOCALL,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 2) AND (b.`id_action_call_track` IS NULL) THEN id_debtor END) AS RETOUCH_NOCALL,
     COUNT( CASE WHEN (skip <> 1 AND is_paid IS NULL AND is_new = 0) AND (b.`id_action_call_track` IS NULL) THEN id_debtor END) AS UNCATEGORIZE_NOCALL
     FROM hdr_debtor_main a
     LEFT JOIN hdr_calltrack b
     ON a.`primary_1` = b.`primary_1` AND b.`call_date` = CURDATE()
     GROUP BY a.valdo_cc
    	";
    	...
    	$delimiter = "|";
					$newline = "\r\n";
    	$csv_layout = $this->dbutil->csv_from_result($qObj, $delimiter, $newline);
