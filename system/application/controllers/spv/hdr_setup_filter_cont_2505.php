...
//die();
  	...
  	foreach($q->result_array() as $row)
  	{
		...
		//reschedule
		$sql = "update hdr_debtor_main set seq_no=1 where last_call_code='OCAA' and last_id_handling_code=15";
		$this->db->query($sql);

		//titip pesan
		$sql = "update hdr_debtor_main set seq_no=2 where last_call_code='OCAA' and last_id_handling_code=1";
		$this->db->query($sql);

		//unit hilang
		$sql = "update hdr_debtor_main set seq_no=3 where last_call_code='OCAA' and last_id_handling_code=7";
		$this->db->query($sql);

		//Noan
		$sql = "update hdr_debtor_main set seq_no=4 where last_call_code='OCNA' and last_id_handling_code=4";
		$this->db->query($sql);

		//line busy
		$sql = "update hdr_debtor_main set seq_no=5 where last_call_code='OCLB' and last_id_handling_code=4";
		$this->db->query($sql);

		//invalid number
		$sql = "update hdr_debtor_main set seq_no=6 where last_call_code='OCIN' and last_id_handling_code=05";
		$this->db->query($sql);
	...
	function database_tracking(){
		...
		COUNT( CASE WHEN is_new = 1 THEN primary_1 END ) AS UNTOUCH,
		COUNT( CASE WHEN (is_new = 1 AND called = 1)  THEN primary_1 END ) AS UNTOUCH_CALLED,
