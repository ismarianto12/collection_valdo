...
class reload extends Controller
{
	...
	function hal1()
	{
		...
		$sql = "select COUNT(CASE WHEN `company` = '0' THEN status END) AS totdat from `campains` where date(tgl) = curdate() limit 1";
		/*$sql = "SELECT date(tgl) as tglnow,
				...
				FROM `campains` AS t1 
				WHERE `company`=0 and date(tgl) = curdate() group by tglnow";*/
		$q = $ems->query($sql);
		//var_dump($ems->last_query());
		$arrdata["ems"] = $q->result_array();
...
function hal3()
	{
		...
		$ems = $this->load->database("ems",true);
		$sql = "select id AS totdat from `campains` where no_kartu = '010416300253' limit 1";
				$q = $ems->query($sql);
		//var_dump($ems->last_query());
		$arrdata["ems"] = $q->result_array();
				
		$this->load->view("page3",$arrdata);
	}
