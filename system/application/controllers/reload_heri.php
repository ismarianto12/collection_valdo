...
class reload extends Controller
{
	...
	{
		$arrdata = array();
		$arrdata["data_top"] = $this->topfive();
		$arrdata["data_top2"] = $this->topfive2();
		$arrdata["worst"] = $this->worstfive();
		$arrdata["worst2"] = $this->worstfive2();
  ...
  function topfive2()
	{
		...
		//Top Five
		$sql = "select * from wallboard_topfive_otomotif order by total_call desc limit 5";
		$q = $this->db->query($sql);
	...
	function worstfive2()
	{
		...
		//Worst Five
		$sql = "select * from wallboard_topfive_otomotif order by total_call asc limit 5";
		$q = $this->db->query($sql);
	...
	function monitoring()
	{
		...
		$sql = "
			...
			SELECT
			  ...
			  ROUND(((COUNT(DISTINCT CASE WHEN hc.id_handling_code = '02' AND ptp_date IS NOT NULL AND ptp_date != '0000-00-00' THEN hc.id_calltrack END ))/(ROUND((0.8*((0.7*(1*350))))))*15),'2') +
			...
			WHERE hc.call_date = '$today' and hc.object_group = '003'
			GROUP BY hc.id_user, hu.id_leader";
		$this->db->query($sql);
		
		//migrate
		$sql = "truncate table wallboard_topfive_otomotif";
		$this->db->query($sql);
		...
		$sql = "
			insert into wallboard_topfive_otomotif
			SELECT
				hc.id_user,
			  hc.username,
			  (ROUND((COUNT(DISTINCT hc.id_calltrack)/(1*350)*5),'2') +
			...
			FROM hdr_calltrack hc
			  ...
			  LEFT JOIN (SELECT
			    ...
			    ON jj.id_user = hc.id_user
			  inner join hdr_user hl on hu.id_leader=hl.id_user
			WHERE hc.call_date = '$today' and hc.object_group in ('001','002') 
			GROUP BY hc.id_user, hu.id_leader";
		$this->db->query($sql);
