...
class reload extends Controller
{
	...
	function hal1()
	{
		...
		$arrdata["data_top"] = $this->topfive(1); //durable
		$arrdata["worst"] = $this->worstfive(1); //durable

		$arrdata["data_top_oto"] = $this->topfive(2); //otomotif
		$arrdata["worst_oto"] = $this->worstfive(2); //otomotif
		...
		$arrdata["debtor_main_invalid"] = $this->Reload_model->od_monitor_calltrack_invalid();
		$arrdata["debtor_main_cust"] = $this->Reload_model->od_monitor_debtor_main_cust();
	...
	function hal_new()
	{
		...
		$this->load->view("page_new",$arrdata);
	}
	...
	function agent_status()
	{
		...
		*/
		try{
            //$st_url = "http://172.25.150.201/cc-adira1/status_agent_adira_new.php";
			$st_url = "http://10.14.14.145/cc-frontend/api/ai_call/cek_status_agent";
        ...
        */
        try{
            //$st_url = "http://172.25.150.201/cc-adira1/status_agent_adira_new.php";
            $st_url = "http://10.14.14.146/cc-frontend/api/ai_call/cek_status_agent";
            $hasil2 = file_get_contents($st_url);
        }catch(Exception $ex)
        {
            echo 'Caught exception: ',  $ex->getMessage(), "\n";
        }
	...
	function topfive($product_flag)
	{
		...
		switch($product_flag)
		{
			case 1:
				//durabel
				$wh .= " and product_flag like '%003,004%' ";
				break;
			case 2:
				//otomotif
				$wh .= " and product_flag like '002,001' ";
				break;		
		}
		
		//Top Five
		$sql = "select id_user,username,sum(total_call) as total_call,leader_name,id_leader,product_flag from wallboard_topfive $wh group by username order by total_call desc limit 5";
		$q = $this->db->query($sql);
//var_dump($this->db->last_query());die();
	...
	function worstfive($product_flag)
	{
		...
		//Worst Five
		$sql = "select id_user,username,sum(total_call) as total_call,leader_name,id_leader,product_flag from wallboard_topfive $wh group by username order by total_call asc limit 5";
		$q = $this->db->query($sql);
		...
		$sql = "
			...
			FROM hdr_calltrack hc
			  ...
			  LEFT JOIN (SELECT
			             ...
			             WHERE date(created) = '$today') hh
			    on hh.user_id = hc.id_user
			  LEFT JOIN (SELECT
			               sum(ptp_amount)      AS sum_ptp_amount,
			               id_user
			             FROM hdr_calltrack
			             WHERE date(call_date) = '$today'
			                 AND ptp_status = '2'
			                 AND id_action_call_track = '11'
			             GROUP BY id_user) jj
			    ON jj.id_user = hc.id_user
			  inner join hdr_user hl on hu.id_leader=hl.id_user
			WHERE date(hc.call_date) = '$today'
			and hdm.valdo_cc='01'
			and hl.blocked=0 
			and hl.id_level=2
			and date(hl.last_login)='$today'			
			GROUP BY hc.id_user, hu.id_leader, hdm.product_flag";

Rumus total data sblmnya dgn bobot target:

			  (ROUND((COUNT(DISTINCT hc.id_calltrack)/(1*300)*5),'2') +
			  ROUND(((COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1') THEN hc.id_calltrack END ))/(0.7*(1*300))*10),'2') +
			  ROUND(((COUNT(DISTINCT CASE WHEN hc.id_handling_code = '02' AND hc.ptp_date IS NOT NULL AND hc.ptp_date != '0000-00-00' THEN hc.id_calltrack END ))/(ROUND((0.8*((0.7*(1*300))))))*15),'2') +
			  ROUND((COUNT(DISTINCT CASE WHEN hc.ptp_status = '2' AND hc.id_handling_code = '02' THEN hc.primary_1 END ))/(ROUND((0.8 * ROUND((0.8*((0.7*(1*300))))))))*70,'2'))
			   as total_data,
			   
			   ROUND(COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1') THEN hc.id_calltrack END )/COUNT(hc.id_calltrack),2) * 100 as total_data,
...
*/
		...
		SELECT
			  hc.id_user,
			  hc.username,
				(COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1') THEN hc.id_calltrack END ) / 300) * (COUNT(distinct(hc.primary_1)))
			   / (COUNT(distinct(hc.primary_1))) * 100 as total_data,
			  hl.username as leader_name,
			  hu.id_leader, hu.product			
			FROM hdr_calltrack hc
			  INNER JOIN hdr_user hu  ON hu.id_user = hc.id_user
			  INNER join hdr_user hl on hu.id_leader=hl.id_user
			WHERE hc.call_date = '$today' 
			GROUP BY hc.id_user, hu.id_leader, hu.product";
		//die($sql);
		$this->db->query($sql);
...
*/
	...
	function insert_blast(){
		...
		$data['urllist1'] = $this->Reload_model->getAllDashboardAdira();
		$data['idlegraph'] = $this->Reload_model->get_data();
