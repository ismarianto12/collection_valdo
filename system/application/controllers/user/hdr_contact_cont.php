...
class Hdr_contact_cont extends Controller
{
    ...
    public function data_call_record()
    {
        ...
        //$data['status_restruktur'] = $this->input->post('status_restruktur', TRUE);
...
'" . $row->ID . "' ,'" . $row->AgreementNo . "',  '" . $row->trx_date . "' , '" . $row->update_date . "', '" . $row->AngsuranKe . "', '" . $row->Amount . "','" . $row->JenisTransaksi . "',CURRENT_TIMESTAMP);
 ";
    ...
    function sp()
    {
        ...
        $data = array(
            array('num' => 1, 'name' => 'No Kartu', 'type' => ': ' . $card_no), array('num' => 2, 'name' => 'Nama', 'type' => ': ' . $name), array('num' => 3, 'name' => 'Total Tagihan', 'type' => ': ' . $balance), array('num' => 4, 'name' => 'Minimum Pembayaran', 'type' => ': ' . $debt_amount)
        );
