<?php
  ...
  class Reload_model extends Model {
      ...
      {
        //syncronize_data
        $sql = "update hdr_debtor_main set is_board=0";
        $this->db->query($sql);

        $sql = "update hdr_debtor_main set is_board=1 where last_call_code='OCIN'";
        $this->db->query($sql);

        $sql = "update hdr_debtor_main set is_board=1 where skip=1";
        $this->db->query($sql);

        $sql = "update hdr_debtor_main set is_board=1 where is_paid=1";
        $this->db->query($sql);

      	$sql = "select
			count(distinct(case when dpd=-3 then primary_1 end)) as total_min3,
      			count(distinct(case when dpd=-3 and is_board=1 then primary_1 end)) as total_min3_skip,
      		sum(case when dpd=-3 then os_ar end) as total_min3_amount,
			count(distinct(case when dpd=0 then primary_1 end)) as total_null,
      			count(distinct(case when dpd=0 and is_board=1 then primary_1 end)) as total_null_skip,
      		sum(case when dpd=0 then os_ar  end) as total_null_amount,
			count(distinct(case when dpd=1 then primary_1 end)) as total_plus1,
      			count(distinct(case when dpd=1 and is_board=1 then primary_1 end)) as total_plus1_skip,
      		sum(case when dpd=1 then os_ar end) as total_plus1_amount,
			count(distinct(case when dpd=4 then primary_1 end)) as total_plus4,
      			count(distinct(case when dpd=4 and is_board=1 then primary_1 end)) as total_plus4_skip,
      		sum(case when dpd=4 then os_ar end) as total_plus4_amount,
			count(distinct(case when dpd=7 then primary_1 end)) as total_plus7,
      			count(distinct(case when dpd=7 and is_board=1 then primary_1 end)) as total_plus7_skip,
      		sum(case when dpd=7 then os_ar end) as total_plus7_amount,
			count(distinct(case when dpd=8 then primary_1 end)) as total_plus8,
      			count(distinct(case when dpd=8 and is_board=1 then primary_1 end)) as total_plus8_skip,
			...
			count(distinct(case when dpd=11 then primary_1 end)) as total_plus11,
      			count(distinct(case when dpd=11 and is_board=1 then primary_1 end)) as total_plus11_skip,
			...
			count(distinct(case when dpd=14 then primary_1 end)) as total_plus14,
      			count(distinct(case when dpd=14 and is_board=1 then primary_1 end)) as total_plus14_skip,
			...
			count(distinct(case when dpd=15 then primary_1 end)) as total_plus15,
      			count(distinct(case when dpd=15 and is_board=1 then primary_1 end)) as total_plus15_skip,
			...
			count(distinct(case when dpd>15 then primary_1 end)) as total_plus15plus,
      			count(distinct(case when dpd>15 and is_board=1 then primary_1 end)) as total_plus15plus_skip,
		...
		//syncronize
      	...
      	$sql = "select
      		...
      		count(case when dpd=10 and is_board=1 then primary_1 end) as total_wk_plus10_skip,
      		...
      		count(case when dpd=12 and is_board=1 then primary_1 end) as total_wk_plus12_skip,
      		...
      		count(case when dpd=13 and is_board=1 then primary_1 end) as total_wk_plus13_skip
      ...
      public function od_monitor_debtor_main_cust()
      {
      	$sql = "select
			...
			count(distinct(case when dpd=0 and object_group_code='003' and is_board=0 then primary_1 end)) as total_null,
      			count(distinct(case when dpd=0 and object_group_code='003' and is_board=1 then primary_1 end)) as total_null_skip,
      		sum(case when dpd=0 and object_group_code='003' and is_board=0 then os_ar  end) as total_null_amount,

			count(distinct(case when dpd=1 and is_board=0 then primary_1 end)) as total_plus1,
      			count(distinct(case when dpd=1 and is_board=1 then primary_1 end)) as total_plus1_skip,
      		sum(case when dpd=1 and is_board=0 then os_ar end) as total_plus1_amount,

			count(distinct(case when dpd=4 and is_board=0 then primary_1 end)) as total_plus4,
      			count(distinct(case when dpd=4 and is_board=1 then primary_1 end)) as total_plus4_skip,
      		sum(case when dpd=4 and is_board=0 then os_ar end) as total_plus4_amount,

			count(distinct(case when dpd=7 and is_board=0 then primary_1 end)) as total_plus7,
      			count(distinct(case when dpd=7 and is_board=1 then primary_1 end)) as total_plus7_skip,
      		sum(case when dpd=7 and is_board=0 then os_ar end) as total_plus7_amount,

			count(distinct(case when dpd=8 and is_board=0 then primary_1 end)) as total_plus8,
      			count(distinct(case when dpd=8 and is_board=1 then primary_1 end)) as total_plus8_skip,
      		sum(case when dpd=8 and is_board=0 then os_ar end) as total_plus8_amount,

			count(distinct(case when dpd=11 and is_board=0 then primary_1 end)) as total_plus11,
      			count(distinct(case when dpd=11 and is_board=1 then primary_1 end)) as total_plus11_skip,
      		sum(case when dpd=11 and is_board=0 then os_ar end) as total_plus11_amount,

			count(distinct(case when dpd=14 and is_board=0 then primary_1 end)) as total_plus14,
      			count(distinct(case when dpd=14 and is_board=1 then primary_1 end)) as total_plus14_skip,
      		sum(case when dpd=14 and is_board=0 then os_ar end) as total_plus14_amount,

			count(distinct(case when dpd=15 and is_board=0 then primary_1 end)) as total_plus15,
      			count(distinct(case when dpd=15 and is_board=1 then primary_1 end)) as total_plus15_skip,
      		sum(case when dpd=15 and is_board=0 then os_ar end) as total_plus15_amount,

			count(distinct(case when dpd>15 and is_board=0 then primary_1 end)) as total_plus15plus,
      			count(distinct(case when dpd>15 and is_board=1 then primary_1 end)) as total_plus15plus_skip,
      		sum(case when dpd>15 and is_board=0 then os_ar end) as total_plus15plus_amount
			from hdr_debtor_main where valdo_cc='01' ";
      	//die($sql);
      	$query = $this->db->query($sql);
      	$data = $query->row_array();
		...
		//syncronize
      	...
      	$sql = "select
      		count(case when dpd=-2 and is_board=0 then primary_1 end) as total_wk_min2,
      		sum(case when dpd=-2 and is_board=0 then os_ar end) as total_wk_min2_amount,
      		count(case when dpd=-2 and is_board=1 then primary_1 end) as total_wk_min2_skip,

      		count(case when dpd=-1 and is_board=0 then primary_1 end) as total_wk_min1,
      		sum(case when dpd=-1 and is_board=0 then os_ar end) as total_wk_min1_amount,
      		count(case when dpd=-1 and is_board=1 then primary_1 end) as total_wk_min1_skip,

      		count(case when dpd=2 and is_board=0 then primary_1 end) as total_wk_plus2,
      		sum(case when dpd=2 and is_board=0 then os_ar end) as total_wk_plus2_amount,
      		count(case when dpd=2 and is_board=1 then primary_1 end) as total_wk_plus2_skip,

      		count(case when dpd=3 and is_board=0 then primary_1 end) as total_wk_plus3,
      		sum(case when dpd=3 and is_board=0 then os_ar end) as total_wk_plus3_amount,
      		count(case when dpd=3 and is_board=1 then primary_1 end) as total_wk_plus3_skip,

      		count(case when dpd=5 and is_board=0 then primary_1 end) as total_wk_plus5,
      		sum(case when dpd=5 and is_board=0 then os_ar end) as total_wk_plus5_amount,
      		count(case when dpd=5 and is_board=1 then primary_1 end) as total_wk_plus5_skip,

      		count(case when dpd=6 and is_board=0 then primary_1 end) as total_wk_plus6,
      		sum(case when dpd=6 and is_board=0 then os_ar end) as total_wk_plus6_amount,
      		count(case when dpd=6 and is_board=1 then primary_1 end) as total_wk_plus6_skip,

      		count(case when dpd=9 and is_board=0 then primary_1 end) as total_wk_plus9,
      		sum(case when dpd=9 and is_board=0 then os_ar end) as total_wk_plus9_amount,
      		count(case when dpd=9 and is_board=1 then primary_1 end) as total_wk_plus9_skip,

      		count(case when dpd=10 and is_board=0 then primary_1 end) as total_wk_plus10,
      		sum(case when dpd=10 and is_board=0 then os_ar end) as total_wk_plus10_amount,
      		count(case when dpd=10 and is_board=1 then primary_1 end) as total_wk_plus10_skip,

      		count(case when dpd=12 and is_board=0 then primary_1 end) as total_wk_plus12,
      		sum(case when dpd=12 and is_board=0 then os_ar end) as total_wk_plus12_amount,
      		count(case when dpd=12 and is_board=1 then primary_1 end) as total_wk_plus12_skip,

      		count(case when dpd=13 and is_board=0 then primary_1 end) as total_wk_plus13,
      		sum(case when dpd=13 and is_board=0 then os_ar end) as total_wk_plus13_amount,
      		count(case when dpd=13 and is_board=1 then primary_1 end) as total_wk_plus13_skip
          ...
          $sql = "select
			...
			count(case when hdm.dpd>15  then hdm.primary_1 end) as total_plus15plus
          	from hdr_debtor_main hdm
          		where hdm.valdo_cc='01' and hdm.id_user>0 and hdm.is_board=0 ";
          $query = $this->db->query($sql);
          $data = $query->row_array();
          ...
          $sql = "select
			...
			count(distinct(case when hdm.dpd>15 then hdm.primary_1 end)) as total_plus15plus
          	from hdr_calltrack hc, hdr_debtor_main hdm
          		...
          		and hc.call_date BETWEEN '{$today} 00:00:00' AND NOW()";
          $query = $this->db->query($sql);
          $data = $query->row_array();
