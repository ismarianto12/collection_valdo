				...
				if($priority != "")
				{
					...
					for($i=0;$i<count($arrdata_prod);$i++)
					{
						switch($arrdata_prod[$i])
						{
							...
							case "PTP_Expired":
							$where .= " and ptp_date_expired < date(now()) and ptp_date_expired is not null and id_user = '$id_user' and month(ptp_date_expired)=month(now()) and year(ptp_date_expired)=year(now()) ";
							//die($where);
							break;
