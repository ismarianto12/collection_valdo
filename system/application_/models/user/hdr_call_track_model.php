...
//die("d" . $over_days);
			...
			*/
				...
				if($is_active == "true")
				{
					...
					//$this->db->query($sql);
						...
						$priority_data = " ";
					$where_leader = "";
						if($id_leader == 772 || $id_leader == 923 || $id_leader == 825 || $id_leader == 902 || $id_leader == 901){
							$where_leader = " and object_group_code in ('001','002','003') and dpd < 7 "; 
						...
						$sql = "select primary_1 from hdr_debtor_main AS lock".$id_user.
                                //" where in_use = 0 and called = 0 and date(createdate) = date(now()) andid_user=$id_user $where_leader $priority_data limit 1";
								" where in_use = 0 and called = 0 and date(createdate) = date(now()) and id_user= $id_user limit 1";
						//die($sql);
						$toupdate = array(
							"id_user" => $id_user,
							"in_use" => "1"
    ...
    public function insert_call_track($data, $insert) {
        ...
        $no_contacted = $data['no_contacted'];
