<?php
  ...
  class Reload_model extends Model {
      ...
      public function od_monitor_debtor_main()
      {
      	...
      	$sql = "select count(distinct(hdm.primary_1)) as total_wk_min1,
      		sum(hdm.os_ar) as total_wk_min1_amount
			from hdr_debtor_main hdm, hdr_calltrack hc
			where hdm.valdo_cc='01' and hdm.primary_1=hc.primary_1
			and date(hc.call_date)=date(INTERVAL -1 DAY + now()) and hdm.dpd=-1";
      	$q1 = $this->db->query($sql);
      	$row1 = $q1->row_array();
      	$total_wk_min1 = $row1['total_wk_min1'];
      	$total_wk_min1_amount = $row1['total_wk_min1_amount'];

      	$sql2 = "select count(primary_1) as total_ms_min1,
      		sum(os_ar) as total_ms_min1_amount
			from hdr_debtor_main_inventory
			where valdo_cc='01' and date(data_date)=date(INTERVAL -1 DAY + now()) and dpd=-1";
      	$q2 = $this->db->query($sql2);
      	$row2 = $q2->row_array();
      	$total_ms_min1 = $row2['total_ms_min1'];
      	$total_ms_min1_amount = $row2['total_ms_min1_amount'];
      	       	
      	$data['total_min2'] = $total_ms_min1 - $total_wk_min1;
      	$data['total_min2_skip'] = 0;
      	$data['total_min2_amount'] = $total_ms_min1_amount - $total_wk_min1_amount;
      	...
      	$sql = "select count(distinct(hdm.primary_1)) as total_wk_min1,
			...
			from hdr_debtor_main hdm, hdr_calltrack hc
			where hdm.valdo_cc='01' and hdm.primary_1=hc.primary_1
			and date(hc.call_date)=date(INTERVAL -2 DAY + now()) and hdm.dpd=0";
      	$q1 = $this->db->query($sql);
      	$row1 = $q1->row_array();
      	$total_wk_min1 = $row1['total_wk_min1'];
      	$total_wk_min1_amount = $row1['total_wk_min1_amount'];
      	...
      	$sql2 = "select count(primary_1) as total_ms_min1,
      		sum(os_ar) as total_ms_min1_amount
			from hdr_debtor_main_inventory
			where valdo_cc='01' and date(data_date)=date(INTERVAL -2 DAY + now()) and dpd=0";
      	$q2 = $this->db->query($sql2);
      	$row2 = $q2->row_array();
      	$total_ms_min1 = $row2['total_ms_min1'];
      	$total_ms_min1_amount = $row2['total_ms_min1_amount'];
      	 
      	$data['total_min1'] = $total_ms_min1 - $total_wk_min1;
      	$data['total_min1_skip'] = 0;
      	$data['total_min1_amount'] = $total_ms_min1_amount - $total_wk_min1_amount;
