...
class Hdr_upload_model extends Model {
       ...
       //$this->debug_query($this->dpd_status());
        ...
        $sql = "UPDATE hdr_debtor_main
								...
								(
									...
									LENGTH(TRIM(LEADING '0' FROM phone_3)) < 5
								) 
								OR 
								(
    ...
    function new_data_recognition() {
		...
		SELECT DISTINCT primary_1 FROM hdr_calltrack WHERE TRIM(primary_1) <> '' AND DATE_FORMAT(call_date,'%Y-%m') = DATE_FORMAT(NOW(),'%Y-%m') 
          )";
       $this->db->simple_query($sql);
       ...
       $sql = "UPDATE hdr_debtor_main 
          		SET is_new = 2 
          		WHERE primary_1 IN (
									SELECT primary_1  FROM hdr_calltrack a WHERE TRIM(primary_1) <> ''
									AND call_date = (SELECT MAX(call_date) FROM hdr_calltrack b WHERE b.primary_1 = a.primary_1)
									AND a.id_handling_code <> '05' AND a.id_handling_code <> '02'
							)";
				$this->db->simple_query($sql);
				unset($sql);
    }
     ...
     function payment_upload($file_path) {
     		$sql = "LOAD DATA LOCAL INFILE '" . $file_path . "' REPLACE INTO TABLE hdr_payment
            		FIELDS TERMINATED BY '|' ESCAPED BY '\\\'  IGNORE 1 LINES (kode_cabang,branch,primary_1,name,angsuran_ke,dpd,amount_pay,amount_ins,amount_due,trx_date,trx_time,entry_date,source,valdo_cc)";
       	//die($sql);
    ...
    function upload_master_temp($file) {
        ...
        (`kode_cabang`,`branch`,`primary_1`,`name`,`angsuran_ke`,`tenor`,`DPD`,`debt_amount`,`amount_due`,`os_ar`,`police_no`,`phone_1`,`phone_2`,`phone_3`,`cell_phone1`,`region`,`product`,`due_date`,`last_handling_date`,`last_remark`,`last_ptp_date`,`last_action`,`last_result`,`object_group_code`,`valdo_cc`,`fin_type`)
		...
		function upload_master()
		{
			...
			(`kode_cabang`,`branch`,`primary_1`,`name`,`angsuran_ke`,`tenor`,`DPD`,`debt_amount`,`amount_due`,`os_ar`,`police_no`,`phone_1`,`phone_2`,`phone_3`,`cell_phone1`,`region`,`product`,`due_date`,`last_handling_date`,`last_remark`,`last_ptp_date`,`last_action`,`last_result`,`object_group_code`,`valdo_cc`,`fin_type`)
			select `kode_cabang`,`branch`,`primary_1`,`name`,`angsuran_ke`,`tenor`,`DPD`,`debt_amount`,`amount_due`,`os_ar`,`police_no`,`phone_1`,`phone_2`,`phone_3`,`cell_phone1`,`region`,`product`,`due_date`,`last_handling_date`,`last_remark`,`last_ptp_date`,`last_action`,`last_result`,`object_group_code`,`valdo_cc`,`fin_type`
			...
			(`kode_cabang`,`branch`,`primary_1`,`name`,`angsuran_ke`,`tenor`,`DPD`,`debt_amount`,`amount_due`,`os_ar`,`police_no`,`phone_1`,`phone_2`,`phone_3`,`cell_phone1`,`region`,`product`,`due_date`,`last_handling_date`,`last_remark`,`last_ptp_date`,`last_action`,`last_result`,`object_group_code`,`valdo_cc`,`data_date`,`fin_type`)
			select `kode_cabang`,`branch`,`primary_1`,`name`,`angsuran_ke`,`tenor`,`DPD`,`debt_amount`,`amount_due`,`os_ar`,`police_no`,`phone_1`,`phone_2`,`phone_3`,`cell_phone1`,`region`,`product`,`due_date`,`last_handling_date`,`last_remark`,`last_ptp_date`,`last_action`,`last_result`,`object_group_code`,`valdo_cc`,DATE_FORMAT(`createdate`,'%Y-%m-%d'),`fin_type`
			from hdr_debtor_main_temp ";
