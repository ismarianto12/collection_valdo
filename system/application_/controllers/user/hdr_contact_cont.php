...
class Hdr_contact_cont extends Controller {
    ...
    public function contact() {
        ...
        $data['session_name'] = $_SESSION['session_name'];        
        $data['api_url'] = $this->config->item('url_asterisk');
        $status_call = $this->cek_proses_call();
            //echo  $status_call;exit;
        $data['status_call_flow'] = $status_call;           
		...
		if (isset($post)) {
            ...
            $primary = $this->uri->segment(5);
            $primary_1 = base64_decode($primary);
            //echo $primary_1;
            //echo  base64_decode($primary_1) ;exit;
...
//            }
            ...
            if ($_SESSION['blevel'] == 'user') :
                if ($primary_1 != "") {
                    if ($type_calling == 'call') {
                        ...
                        $get_last = $no_loan."_".DATE("i");
				        //echo base64_decode($get_last);exit;
                         
                
                		//$get_last = base64_encode($primary_1."_".DATE("i"));
                        //echo $get_last;exit;
						$dtnow = date("Y-m-d");
						$sql = "update hdr_debtor_main hdm, hdr_calltrack hc
								set hdm.called=1
								where hc.call_date = '$dtnow'
								and hdm.primary_1='$no_loan'
						...
						/*if($no_loan != 0){
							...
							$call_noans = base64_encode($this->call_track_model->get_call_noans()."_".DATE("i"));
							redirect('user/hdr_contact_cont/contact/' .$type_calling . '/' . $call_noans);
						}*/
		...
		if($primary_1 == 0 || $primary_1 == '0' || $primary_1 == ''){
			$this->load->view('user/hdr_header_user_pd', $data);
			$this->load->view('user/hdr_contact_view_kosong');
			...
			a.primary_1 = '$primary_1' and a.ptp_date IS NOT NULL and a.ptp_date >= DATE(NOW())  LIMIT 1";
			...
			*/
			...
			if($result->num_rows() > 0) {
				$row = $result->row();
				if($row->ptp_status == 0) { 
					$data['last_ptp_status'] = "A"; 
				} // Active
				else if ($row->ptp_status == 1 ) { 
					$data['last_ptp_status'] = "B"; 
				} // Broken
				else { 
					$data['last_ptp_status'] = "K"; 
				} // Keep
			} else { 
				$data['last_ptp_status'] = "false"; 
			}
			################################
			...
			$data['check_ptp'] = $this->call_track_model->check_ptp($primary_1);
        ...
        if (isset($primary_1)) {
            ...
            //$this->db->simple_query($sql_lock);
				...
				if(!$query || $query->num_rows() == 0){
					...
					$last_call_code = $this->input->post('last_call_code', TRUE);
    ...
    public function data_call_record() {
        ...
        //$data['telp_hp'] = strtoupper($this->input->post('telp_hp', TRUE));
	...
	$data['score_result'] = $this->input->post('score_result', TRUE);
        ...
        //var_dump($data['no_contacted']);die();
...
'".$row->ID."' ,'" . $row->AgreementNo . "',  '" . $row->trx_date . "' , '" . $row->update_date . "', '" . $row->AngsuranKe . "', '" . $row->Amount . "','" . $row->JenisTransaksi . "',CURRENT_TIMESTAMP);
 ";
    ...
    public function insert_calltrack_rep() {
        //$this->output->enable_profiler("TRUE");
        $primary_1 = $_POST['primary_1'];
	 ...
	 $id_user = $data['id_user'];
							...
							$this->call_track_model->insert_call_track_rep($data, $insert = '1');
							$this->call_track_model->in_use_debtor($primary_1, $id_user, $not_ptp = 0);
           }
    ...
    public function keluar() {
		...
		$data['get_signout_option'] = $this->call_track_model->get_signout_option();
			...
			if(intval($data['fin_type']) == 2){
				...
				$this->load->view('user/hdr_header_user_keluar',$data);
			}
			$this->load->view('user/hdr_contact_view_logout',$data);
			if(intval($data['fin_type']) == 2){
				$this->load->view('user/hdr_footer_syr',$data);
			}else {
				$this->load->view('user/hdr_footer');
			}
		}
    
 public function refresh_predictive(){
     ...
     //echo 'test';
        ...
        $data['level'] = "TEst";
        $data['session_name'] = $_SESSION['session_name'];        
        $data['extension'] = $_SESSION['extension'];        
        $data['get_reminder_new'] = array();
                   
        $data['div_state'] = 'none';
        
        $id_user = $_SESSION['bid_user_s'];
        //echo $id_user; 
//        $data['get_user_detail'] = $this->call_track_model->get_user_detail($id_user);
//        $data['get_signout_option'] = $this->call_track_model->get_signout_option();
        $data['api_url'] = $this->config->item('url_asterisk');
        ...
        $this->load->view('user/hdr_header_user_pd', $data);
        //$this->load->view('user/hdr_footer_syr', $data);
        $this->load->view('user/hdr_pd', $data);
        $this->load->view('user/hdr_footer', $data);

    }
    
 public function update_getdata(){
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
        ...
        //clean data
        /*
        $table = 'hdr_debtor_main';
        $where = "Where called = 0 and id_user = '$id_user' limit 1";
        $field_value = " called = 1,
                         id_user = 0 ";
        $get = $this->M_crud->proses_update($table,$field_value,$where);                  
        */
            
        $table = 'hdr_debtor_main';
        $where = "Where primary_1 = '$primary_1'  limit 1";
        $field_value = " id_user = '$id_user', 
                         called = 0";
 ...
 function cek_proses_call(){
        $where = "where ids = 1 "; //
        $field = " proses ";
        $rslt_dt = $this->M_crud->proses_select_rows_fields_dc('t_proses_parameter',$where,$field); 
 ...
 function get_manual_called(){

     
        $where = "Where (id_user = 0 or id_user is null) and called = 1 "; //
        $field = " primary_1 ";
        $rslt_dt = $this->M_crud->proses_select_rows_fields_dc('hdr_debtor_main',$where,$field); 
        ...
        $primary_1 = "";
                    ...
                    if($rslt_dt <> 'false')
                    {                                  
                        $primary_1 = $rslt_dt->primary_1;
                    }
                    
     $result['primary_1'] = $primary_1;              
     $result['primary_enc'] = base64_encode($primary_1);              
