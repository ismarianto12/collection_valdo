...
class Hdr_contact_cont extends Controller {
    ...
    public function contact() {
        ...
        $getstr = $this->call_track_model->debtor_details($primary_1);
				...
				a.primary_1 = '$primary_1' AND a.ptp_date IS NOT NULL AND a.ptp_date >= DATE(NOW())and s.dpd not in(8,16,7,4,5,6,7) and s.`score_result`!='HIGH'  LIMIT 1";
				
				$result = $this->db->query($sql);
				if($result->num_rows() > 0) {
				$row = $result->row();
						
					if($row->ptp_status == 0 && $row->dpd != 8) {$data['last_ptp_status'] = "A"; } // Active
					else if($row->ptp_status == 0 && $row->dpd == 8){$data['last_ptp_status'] = "B";}
					else if($row->ptp_status == 0 && $row->dpd == 7 && $row->score_result == 'HIGH'){$data['last_ptp_status'] = "B";}
					else if($row->ptp_status == 0 && $row->dpd == 16 && $row->score_result == 'HIGH'){$data['last_ptp_status'] = "B";}
