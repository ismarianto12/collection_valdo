...
//die("dorrr");
		...
		public function read_payment2()
    {
		...
		function link_report1(){
			//die('haha');
			$file_now = "DPD1.txt";
			$sql = "SELECT @no:=@no+1 dealer_id, primary_1 AS no_kartu, LOWER( NAME ) AS nama, IF( cell_phone1 =0, phone_1, cell_phone1 ) AS New_phone, angsuran_ke AS angsuran, substring_index( lower( product ) , '/', 1 ) AS jenis, substring_index( lower( product ) , '/', -1 ) AS merk, police_no AS nopol, due_date AS jatuh_tempo,shift AS minimum, debt_amount AS tagihan, kode_cabang
		...
		function link_report2(){
			$file_now = "DPD-3.txt";
			$sql = "SELECT @no:=@no+1 dealer_id, primary_1 AS no_kartu, LOWER( NAME ) AS nama, IF( cell_phone1 =0, phone_1, cell_phone1 ) AS New_phone, angsuran_ke AS angsuran, substring_index( lower( product ) , '/', 1 ) AS jenis, substring_index( lower( product ) , '/', -1 ) AS merk, police_no AS nopol, due_date AS jatuh_tempo,shift AS minimum, debt_amount AS tagihan, kode_cabang
				FROM `hdr_debtor_main`, (SELECT @no:= 0) AS NO WHERE dpd ='-3' and skip='1' and cell_phone1 != '-' and phone_1 != '-' and cell_phone1 != '' and phone_1 != '' and cell_phone1 != 'TIDAK ADA' and phone_1 != 'TIDAK ADA' and substr(cell_phone1, 1, 2) != '00' and substr(phone_1, 1, 2) != '00' and cell_phone1 != '0' and phone_1 != '0'";
			$q = $this->db->query($sql);
			...
			$sql = "SELECT @no:=@no+1 dealer_id, primary_1 AS no_kartu, LOWER( NAME ) AS nama, IF( cell_phone1 =0, phone_1, cell_phone1 ) AS New_phone, angsuran_ke AS angsuran, substring_index( lower( product ) , '/', 1 ) AS jenis, substring_index( lower( product ) , '/', -1 ) AS merk, police_no AS nopol, due_date AS jatuh_tempo,shift AS minimum, debt_amount AS tagihan, kode_cabang
                	FROM hdr_debtor_main, (SELECT @no:= 0) AS NO
                	WHERE dpd = '-2' and skip='1' and cell_phone1 != '-' and phone_1 != '-' and cell_phone1 != '' and phone_1 != '' and cell_phone1 != 'TIDAK ADA' and phone_1 != 'TIDAK ADA' and substr(cell_phone1, 1, 2) != '00' and substr(cell_phone1, 1, 2) != '+6' and substr(cell_phone1, 1, 2) != '62' and substr(phone_1, 1, 2) != '00' and cell_phone1 != '0' and phone_1 != '0' 
               	 ";
		...
		function link_report4(){
			...
			$sql = "SELECT @no:=@no+1 dealer_id, primary_1 AS no_kartu, LOWER( NAME ) AS nama, IF( cell_phone1 =0, phone_1, cell_phone1 ) AS New_phone, angsuran_ke AS angsuran, substring_index( lower( product ) , '/', 1 ) AS jenis, substring_index( lower( product ) , '/', -1 ) AS merk, police_no AS nopol, due_date AS jatuh_tempo,shift AS minimum, debt_amount AS tagihan, kode_cabang
                	FROM hdr_debtor_main, (SELECT @no:= 0) AS NO
                	WHERE dpd = '0' and skip='1' and cell_phone1 != '-' and phone_1 != '-' and cell_phone1 != '' and phone_1 != '' and cell_phone1 != 'TIDAK ADA' and phone_1 != 'TIDAK ADA' and substr(cell_phone1, 1, 2) != '00' and substr(cell_phone1, 1, 2) != '+6' and substr(cell_phone1, 1, 2) != '62' and substr(phone_1, 1, 2) != '00' and cell_phone1 != '0' and phone_1 != '0' 
               	 ";
