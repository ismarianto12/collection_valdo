...
<script>
	...
	$(document).ready(function(){
		...
		var tahun = $('.tahun').val();
		var area = $('.area').val();
		$('.sum_report').load('<?=site_url()?>spv/hdr_spv_report_ctrl/report_adira_view/'+bulan+'/'+tahun+'/'+area)
	});
	function reloadSumCases(bulan,tahun,area){	
		$(document).ready(function(){
			$('.sum_report').load('<?=site_url()?>spv/hdr_spv_report_ctrl/report_adira_view/'+bulan+'/'+tahun+'/'+area)	
		});	
	}
...
<div class="content">
	<div class="cnfull">
		...
		<form action="<?php echo site_url();?>spv/hdr_spv_report_ctrl/report_adira_view/" method="post" id="Fm">
			...
			</select>&nbsp;&nbsp;&nbsp;<input type="button" value="GO / Refresh" class="but_reg" onclick="goSumCases();return false;">&nbsp;&nbsp;&nbsp;<input type="button" value="Export To Excel" class="but_reg" onclick="jQuery('#export').val(1);jQuery('#Fm').submit()">
