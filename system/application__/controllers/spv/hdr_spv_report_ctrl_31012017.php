...
class Hdr_spv_report_ctrl extends Controller {
 ...
 public function get_handling($begindate,$enddate){
    	$file_now = "RH_WO".date("Y_m_d_Gis").".txt";
    	$sql = "
	...
	IFNULL(c.description,CASE a.id_handling_code WHEN '04' THEN 'NO ANSWER' WHEN '03' THEN 'BUSY LINE' WHEN '05' 
	THEN 'Invalid Number' ELSE '' END) AS 'RESULT HANDLING', a.no_contacted AS 'NOTELP',case 
	when left(a.no_contacted,2)='08' then 'Handphone' else 'Rumah' end as ACTION ,
	CASE WHEN a.id_call_cat = 1 THEN 'Konsumen' 
	 ELSE '' END AS HANDLING_DEBITUR,CASE WHEN a.id_call_cat = 1 THEN 'ADA' 
	 ELSE '' END AS STATUS_UNIT, CASE WHEN a.id_call_cat = 1 
	 THEN 'ADA' ELSE '' END AS STATUS_DEBITUR FROM adira_collectionv2.hdr_calltrack a
	LEFT JOIN adira_collectionv2.hdr_branches b
	ON a.kode_cabang = b.area_code
	LEFT JOIN adira_collectionv2.hdr_call_catagory c
	ON a.id_handling_code = c.code
        LEFT JOIN adira_collectionv2.hdr_contact_code s
        on a.id_contact_code = s.code
	WHERE 
	a.call_date BETWEEN '$begindate' AND '$enddate'
	AND a.createdate = (
	SELECT MAX(c.createdate) FROM adira_collectionv2.hdr_calltrack c WHERE c.primary_1 = a.primary_1 AND
