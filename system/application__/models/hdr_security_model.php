  ...
  class Hdr_security_model extends Model {
      ...
      function login($get_user) {
          ...
          if (!empty($data)) {
              ...
              /* Asterisk Session */
              $_SESSION['ext'] = $data->pabx_ext;
              $_SESSION['queue'] = $data->queue;
              $_SESSION['id_user'] = $data->id_asterisk;
              $_SESSION['leave_reason'] = $data->leave_reason;

              $new_log['user_status'] = 'online';
              $new_log['last_login'] = $_SESSION['last_login'];
              
              
              if($detail_level->level == 'user'){
                $last_leave = (@$data->datetime_leave);
                $last_leave_unix = strtotime($last_leave); 
                $last_leave_date = DATE("Y-m-d", $last_leave_unix);
                if($last_leave_date == DATE("Y-m-d")){
                    $toUpdate = array(
                       'datetime_relogin'=> DATE("Y-m-d H:i:s")
                    );
                    $this->db->where('id_user', $data->id_user);
                    !empty($data->id_user) ? $this->db->update('hdr_user', $toUpdate) : '';
                }
              }
