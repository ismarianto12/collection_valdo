    ...
    public function get_dpds($dpd) {
        ...
        */
				...
				/*if($score_result == 1)
					...
					{
						$where .= " and score_result in('HIGH','MEDIUM','LOW')";
					}else{
...
/*
				...
				{					
					for($i=0;$i<count($angs_ke);$i++)
					{
						...
						{
							case "6":
							$where .= " and angsuran_ke <= '6' ";
							//die($where);
							break;

							case "12":
								$where .= " and angsuran_ke between '7'and '12' ";
								//die($where);
							break;

							case "18":
								$where .= " and angsuran_ke between '13'and '18' ";
							break;
							
							case "24":
							$where .= " and angsuran_ke between '19'and '24' ";
							//die($where);
							break;

							case "30":
							$where .= " and angsuran_ke between '25'and '30' ";
							//die($where);
							break;

							case "36":
								$where .= " and angsuran_ke between '31'and '36' ";
							break;

							case "37":
								$where .= " and angsuran_ke >36 ";
							break;
						}
					}
				}
				...
				if($is_active == "true")
				{
					...
					//$this->db->query($sql);
						...
						$sql = "select primary_1 from hdr_debtor_main AS lock".$id_user.
								" where in_use = 0 and called = 0 and date(createdate) = date(now()) and (id_user = 0 or id_user=$id_user) $where_leader $priority_data limit 1";
						//die($sql);
						$toupdate = array(
							"id_user" => $id_user,
							"in_use" => "1"
...
AND called =0";
    ...
    function summary_prod() {
        ...
        $sql = "select username,id_user,call_date,
        COUNT(*) AS 'call',
        COUNT(DISTINCT(primary_1)) AS 'work',
        COUNT(DISTINCT CASE WHEN (id_call_cat='1') THEN id_calltrack  END ) AS 'contact',
        COUNT(DISTINCT CASE WHEN (id_call_cat='2') THEN id_calltrack  END ) AS 'no_contact',
        COUNT(DISTINCT CASE WHEN (id_handling_code = '02' AND ptp_date IS NOT NULL and ptp_date != '0000-00-00') THEN id_calltrack  END ) AS 'ptp',
        COUNT(DISTINCT CASE WHEN (ptp_status = '2' AND id_handling_code = '02') THEN primary_1  END ) AS 'keep',
        COUNT(DISTINCT CASE WHEN (ptp_status = '1' AND id_handling_code = '02') THEN primary_1  END ) AS 'broken'
        from hdr_calltrack
    ...
    public function ptp_catagory() {
	   ...
	   $sql = "select * from hdr_call_catagory where `type` = 'P' and id_contact_code not in (70,89)";
	   $query = $this->db->query($sql);
       return $query;
    }
    ...
    public function get_call_track_info($primary_1) {
        $sql = "SELECT DISTINCT a.ptp_status, a.call_date, a.username, s.score_result, s.dpd, s.ptp_date FROM hdr_calltrack a, hdr_debtor_main s
...
//var_dump($range);
    ...
    public function cekdata($primary_2) {//var_dump($primary_2);die();
        $sql =  "SELECT a.primary_1 as value
FROM hdr_calltrack a, hdr_debtor_main s
WHERE a.primary_1 = s.primary_1 and a.primary_1 = '$primary_2'
AND a.call_date =curdate()";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->row();
        } else {
