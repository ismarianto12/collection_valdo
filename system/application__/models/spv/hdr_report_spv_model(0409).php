  ...
  class Hdr_report_spv_model extends Model {
      ...
      public function status_call($user, $id_user, $status, $begindate, $enddate) {
          ...
          $sql = " SELECT
                      ...
                      '$enddate' as 'enddate',
                      (350*$total_days) AS target_work,
                      COUNT(DISTINCT hc.id_calltrack) as acct_work,
                      ROUND((COUNT(DISTINCT hc.id_calltrack)/($total_days*350)*5),'2') AS 'arch_work',
                      ROUND((0.7*($total_days*350))) AS 'target_contact',
                      COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1')
                      	THEN  hc.id_calltrack  END ) AS 'contact',
                      ROUND(((COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1') THEN hc.id_calltrack END ))/(0.7*($total_days*350))*10),'2') AS 'arch_contact',
                      COUNT(DISTINCT CASE WHEN (hc.id_call_cat='2')
                      	THEN hc.id_calltrack  END ) AS 'no_contact',
                      ROUND((0.8*((0.7*($total_days*350))))) AS 'target_ptp',
                      ...
                      ROUND(((COUNT(DISTINCT CASE WHEN hc.id_handling_code = '02' AND ptp_date IS NOT NULL AND ptp_date != '0000-00-00' THEN hc.id_calltrack END ))/(ROUND((0.8*((0.7*($total_days*350))))))*15),'2') AS 'arch_ptp',
                      ROUND((0.8*(ROUND((0.8*((0.7*($total_days*350)))))))) AS 'target_keep',
                      COUNT(DISTINCT CASE WHEN hc.ptp_status = '2' AND hc.id_handling_code = '02' THEN hc.primary_1 END ) AS 'keep',
                      ROUND((COUNT(DISTINCT CASE WHEN hc.ptp_status = '2' AND hc.id_handling_code = '02' THEN hc.primary_1 END ))/(ROUND((0.8 * ROUND((0.8*((0.7*($total_days*350))))))))*70,'2') AS 'arch_keep',
      ...
      public function count_assign_debtor_tc($id_user) {
          ...
          */
					...
					if($product_flag != "")
					{
						$arr_temp = array();
						$where .= " and (";
						$arrdata = explode(",", $product_flag);
						$arrdata2 = explode(",", $product_flag);
						for($i=0;$i<count($arrdata);$i++)
						{
							$arr_temp[] .= " product_flag = (".$arrdata[$i].") ";
						}
						$where .= implode("or",$arr_temp) . ")";
						//die($where);
					}
					...
					if($bucket_coll != "")
					{
						$arr_temp = array();
						$where .= " and (";
						$arrdata = explode(",", $bucket_coll);
						$arrdata2 = explode(",", $bucket_coll);
						for($i=0;$i<count($arrdata);$i++)
						{
							$arr_temp[] .= " bucket_coll = (".$arrdata[$i].") ";
						}
						$where .= implode("or",$arr_temp) . ")";
					}
					...
					if($priority != "")
					{
						...
						for($i=0;$i<count($arrdata_prod);$i++)
						{
							switch($arrdata_prod[$i])
							{
								...
								case "RETOUCH":
									$where .= " and is_new = 2 ";
									$where .= " and called = 1 and date(call_date)<date(now()) ";
