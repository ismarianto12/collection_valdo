  ...
  class Hdr_report_spv_model extends Model {
      ...
      public function status_call($user, $id_user, $status, $begindate, $enddate) {
          ...
          $sql = " SELECT
                      ...
                      COUNT(DISTINCT hc.primary_1) as acct_work,
                      ROUND((COUNT(DISTINCT hc.primary_1)/($total_days*$xtarget)*5),'2') AS 'arch_work',
                      ROUND((0.7*($total_days*$xtarget))) AS 'target_contact',
                      COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1')
                      	THEN  hc.primary_1  END ) AS 'contact',
                      ROUND(((COUNT(DISTINCT CASE WHEN (hc.id_call_cat='1') THEN hc.primary_1 END ))/(0.7*($total_days*$xtarget))*10),'2') AS 'arch_contact',
                      COUNT(DISTINCT CASE WHEN (hc.id_call_cat='2')
                      	THEN hc.primary_1  END ) AS 'no_contact',
                      ROUND((0.8*((0.7*($total_days*$xtarget))))) AS 'target_ptp',
                      COUNT(DISTINCT CASE WHEN hc.id_handling_code = '02' AND ptp_date IS NOT NULL and ptp_date != '0000-00-00' THEN hc.primary_1 END ) AS 'ptp',
                      ROUND(((COUNT(DISTINCT CASE WHEN hc.id_handling_code = '02' AND ptp_date IS NOT NULL AND ptp_date != '0000-00-00' THEN hc.primary_1 END ))/(ROUND((0.8*((0.7*($total_days*$xtarget))))))*15),'2') AS 'arch_ptp',
      ...
      */
      ...
      $fetch = mysql_fetch_array($result); $val = $fetch['id_leader'];
          ...
          if ($begindate != "" && $status != 'AC') {
              $bg = " AND hdc.call_date>='$begindate ' ";
			  $bg2 = " AND call_date>='$begindate ' ";
          } else {
              $bg = " ";
			  $bg2 = " ";
          }
          if ($enddate != "" && $status != 'AC') {
              $ed = " AND hdc.call_date<='$enddate' AND hdc.id_spv='$val'";
			  $ed2 = " AND call_date<='$enddate' AND id_spv='$val'";
          } else {
              $ed = " ";
			  $ed2 = " ";
          }
        $user_s = $id_user != 'all' ? "AND hdc.id_user   = '$id_user'" : '';
		$user_s2 = $id_user != 'all' ? "AND id_user   = '$id_user'" : '';
          if ($id_user == 'all') {
              $user_s = "";
			  $user_s2 = "";
          }
          if ($status == "ptp") {
			  ...
			  $status_r2 = " id_handling_code ='02' AND ptp_date IS NOT NULL AND ptp_date != '0000-00-00' ";
          } elseif ($status == "acc") {
              //$group_bys = 'hdc.id_calltrack';
			  ...
			  $status_r2 = "  (id_call_cat='1')";
          } elseif ($status == '2') {
			  $group_bys = 'hdc.primary_1';
              $status_r = "  (hdc.id_call_cat='2')";
			  $status_r2 = "  (id_call_cat='2')";
          } elseif ($status == 'all') {
		...
		WHERE $status_r
			...
			$querys['main_query'] = " SELECT hdc.*,CASE hdc.ptp_status    WHEN '0' THEN 'N/A' WHEN '1' THEN 'Broken' WHEN '2' THEN 'KEEP' END as 'ptp_stat',
                MAX(hdp.trx_date) as max_date, MAX(hdp.amount_pay) as max_amount, hdm.dpd as mdpd, hdc.ptp_date as cptp_date, hdc.primary_1 as en_primary_1, hdc.code_call_track as hdctrack
		FROM (select * from hdr_calltrack WHERE $status_r2 $bg2 $ed2 $user_s2 order by createdate DESC ) AS hdc
                LEFT JOIN hdr_payment hdp ON hdc.primary_1 = hdp.primary_1
                LEFT JOIN hdr_debtor_main hdm  ON hdc.primary_1 = hdm.primary_1
		WHERE $status_r
                $bg $ed
...
*/
      ...
      public function report_status_to_csv($id_user, $status, $begindate, $enddate, $report) {
          ...
          if ($enddate != "" && $status != 'AC' && $status != 'untc') {
              $ed = " AND hdc.call_date<='$enddate' ";
			  $ed2 = " AND call_date<='$enddate' ";
          } else {
              $ed = " ";
			  $ed2 = " ";
          }
          $user_s = $_SESSION['bid_user_s'] != '1' ? "WHERE hdc.id_spv   = '" . $_SESSION['bid_user_s'] . "'  " : 'WHERE hdc.primary_1 !="" ';
		  $user_s2 = $_SESSION['bid_user_s'] != '1' ? "WHERE id_spv   = '" . $_SESSION['bid_user_s'] . "'  " : 'WHERE hdc.primary_1 !="" ';

          $user_id = $id_user != 'all' ? ' AND hdc.id_user ="' . $id_user . '"' : '';
		  $user_id2 = $id_user != 'all' ? ' AND id_user ="' . $id_user . '"' : '';
          if ($status == "ptp") {
              $group_bys = 'hdc.id_calltrack';
              $status_r = "AND hdc.id_handling_code ='02' AND hdc.ptp_date IS NOT NULL and hdc.ptp_date != '0000-00-00'";
			  $status_r2 = "AND id_handling_code ='02' AND ptp_date IS NOT NULL and ptp_date != '0000-00-00'";
              //$bg = " AND hdc.ptp_date>='$begindate ' ";
              //$ed = " AND hdc.ptp_date<='$enddate' ";
          } elseif ($status == "all") {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_action_call_track !='0' ";
			  $status_r2 = "AND id_action_call_track !='0' ";
          } elseif ($status == "acc") {
              $group_bys = 'hdc.primary_1';
              $status_r = "   ";
			  $status_r2 = "   ";
			  ...
			  $status_r2 = "AND (id_call_cat='1')";
          } elseif ($status == "2") {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND (hdc.id_call_cat='2')";
			  $status_r2 = "AND (id_call_cat='2')";
          } elseif ($status == 'AC') {
              $status_r = " ";
			  $status_r2 = " ";
              $group_bys = 'hdp.primary_1';
              ...
              $status_r = "AND hdc.id_handling_code ='02' AND  hdc.ptp_status ='2'
                 AND MONTH(hdp.trx_date) >= MONTH(hdc.call_date) ";
				$status_r2 = "AND hdc.id_handling_code ='02' AND  hdc.ptp_status ='2' ";
          } elseif ($status == 'broken') {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_handling_code ='02' AND  hdc.ptp_status ='1'   ";
			  $status_r2 = "AND id_handling_code ='02' AND  ptp_status ='1'   ";
          }elseif ($status == 'na') {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_handling_code ='02' AND  hdc.ptp_status ='0'   ";
			  $status_r2 = "AND id_handling_code ='02' AND  ptp_status ='0'   ";
          }
          //echo $status;
          $ptp_status1 = $status == 'ptp' ? "'PTP Status'," : "";
          $ptp_status2 = $status == 'ptp' ? "CASE ptp_status WHEN '1' THEN 'BROKEN' WHEN '2' THEN 'KEEP' END  as ptp_status," : "";
          ...
          $file_now = 'calltrack_' . $_SESSION['bsname_s'] . '_' . date('Ymd-H_i_s') . '.txt';
          $sql = "SELECT 'branch_id','branch_name','contract_no','customer_name','overdue','installment','promised','action','handphone','action_description','handling_debitur','desc_handling','result_handling','handling_status','ptp_status','status unit', 'status_debitur', 'PIC_Handling', 'tanggal_result', 'keterangan'
        UNION
        SELECT
		...
		hdc.no_contacted AS 'handphone',
              ...
              case hdc.id_handling_debt when '01' then 'TITIP PESAN' when '02' then 'Janji Bayar' when '06' then 'CUSTOMER TIDAK DIKENAL' when '07' then 'CUSTOMER KEBERATAN MEMBAYAR' when '08' then 'CUSTOMER PINDAH ALAMAT' when '09' then 'BAYAR VIA ATM BELUM TERBUKU' when '10' then 'BAYAR VIA POS BELUM TERBUKU' when '11' then 'BAYAR VIA DEALER BELUM TERBUKU' when '12' then 'BAYAR VIA CMO ATAU ARO BELUM TERBUKU' when '13' then 'BAYAR VIA KASIR BELUM TERBUKU' when '14' then 'JANJI RESCHEDULE' when '04' then 'NO TELP SIBUK/TIDAK DIANGKAT' when '05' then 'NO TELP TIDAK VALID/RUSAK' ELSE 'NA' END as 'desc_handling',
              hdc.id_contact_code AS 'result_handling',
              CASE hdc.id_contact_code WHEN '01' THEN 'ISTRI/SUAMI' WHEN '02' THEN 'SAUDARA' WHEN '03' THEN 'PEMBANTU/SECURITY' WHEN '04' THEN 'FAMILY' WHEN '05' THEN 'ANAK' WHEN '06' THEN 'TETANGGA' WHEN '07' THEN 'BELUM DIKETAHUI' WHEN '08' THEN 'PIHAK LAIN' WHEN '00' THEN 'CUSTOMER' ELSE 'NA' END  as 'handling_status',
              CASE ptp_status WHEN '1' THEN 'BROKEN' WHEN '2' THEN 'KEEP' ELSE 'NA' END  as ptp_status,
            ...
            FROM (select * from hdr_calltrack $user_s2 $user_id2 $status_r2 $bg2 $ed2  order by createdate DESC ) AS hdc
              INNER JOIN hdr_action_call_track hda ON hdc.id_action_call_track = hda.id_action_call_track
              LEFT  JOIN hdr_payment AS hdp
                ON hdp.primary_1 = hdc.primary_1
              LEFT JOIN hdr_user AS hu ON hdc.id_user = hu.id_user
       ...
       public function report_status_to_csv_spv($id_user, $status, $begindate, $enddate, $report) {
          ...
          $user_s = $_SESSION['bid_user_s'] != '1' ? "WHERE hdc.id_spv   = '" . $_SESSION['bid_user_s'] . "'  " : 'WHERE hdc.primary_1 !="" ';
		  $user_s2 = $_SESSION['bid_user_s'] != '1' ? "WHERE id_spv   = '" . $_SESSION['bid_user_s'] . "'  " : 'WHERE primary_1 !="" ';

          $user_id = $id_user != 'all' ? '' : '';
		  $user_id2 = $id_user != 'all' ? '' : '';
          if ($status == "ptp") {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_handling_code ='02' AND hdc.ptp_date IS NOT NULL and hdc.ptp_date != '0000-00-00'";
			  $status_r2 = "AND id_handling_code ='02' AND ptp_date IS NOT NULL and ptp_date != '0000-00-00'";
              ...
              $status_r = "AND hdc.id_action_call_track ='11' AND  hdc.ptp_status ='2'
                 AND MONTH(hdp.trx_date) >= MONTH(hdc.call_date) ";
				 $status_r2 = "AND hdc.id_action_call_track ='11' AND  hdc.ptp_status ='2' ";
          } elseif ($status == 'broken') {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_action_call_track ='11' AND  hdc.ptp_status ='1'   ";
			  $status_r2 = "AND id_action_call_track ='11' AND  ptp_status ='1'   ";
          }elseif ($status == 'na') {
              $group_bys = 'hdc.primary_1';
              $status_r = "AND hdc.id_action_call_track ='11' AND  hdc.ptp_status ='0'   ";
			  $status_r2 = "AND id_action_call_track ='11' AND  ptp_status ='0'   ";
          }
          //echo $status;
          $ptp_status1 = $status == 'ptp' ? "'PTP Status'," : "";
          $ptp_status2 = $status == 'ptp' ? "CASE ptp_status WHEN '1' THEN 'BROKEN' WHEN '2' THEN 'KEEP' END  as ptp_status," : "";
      ...
      public function status_total() {
          
		$db1 = $this->load->database("db_vb",true);
		
          $sql = " select count(*)as tootal from campains ";
